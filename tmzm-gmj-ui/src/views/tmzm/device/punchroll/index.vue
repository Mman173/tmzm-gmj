<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain icon="Top" :disabled="multiple" @click="handleAssemble">上线</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="Bottom" :disabled="multiple" @click="handleReserve">下线</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <!-- 数据表格 -->
    <el-table v-loading="loading" :data="punchrollList" @selection-change="handleSelectionChange" stripe border>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="650px" append-to-body>
      <el-form ref="punchrollRef" :model="form" :rules="rules" label-width="100px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="转鼓号" prop="rotatingDrumNo">
...
import { listPunchroll, getPunchroll, insertPunchroll, updatePunchroll, scrapPunchroll, onlinePunchroll, offlinePunchroll } from '@/api/tmzm/device/punchroll'
import { ElMessage, ElMessageBox } from 'element-plus'

const punchrollList = ref([])
...
const rules = {
  rollerNoTr: [{ required: true, message: '穿孔辊号不能为空', trigger: 'blur' }]
}
...
const getList = async () => {
  loading.value = true
  try {
    const res = await listPunchroll(queryParams)
    punchrollList.value = res.rows
...
/** 提交 */
const submitForm = () => {
  punchrollRef.value.validate(valid => {
    if (valid) {
      if (form.recId) {
        updatePunchroll(form).then(() => {
          ElMessage.success('修改成功')
          dialog.visible = false
          getList()
        })
      } else {
        insertPunchroll(form).then(() => {
...
const handleAssemble = () => {
  ...
  ElMessageBox.confirm('是否确认上线选中的穿孔辊？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    return onlinePunchroll(recIds)
  }).then(() => {
    getList()
...
const handleReserve = () => {
  const recIds = ids.value.join(',')
  ElMessageBox.confirm('是否确认下线选中的穿孔辊？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    return offlinePunchroll(recIds)
  }).then(() => {
    getList()
...
const reset = () => {
  ...
  if (punchrollRef.value) {
    punchrollRef.value.resetFields()
  }
}
